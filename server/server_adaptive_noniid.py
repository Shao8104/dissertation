...
CLIENT_DETAIL_FILE = os.path.join(
    RESULTS_DIR,
    "adaptive_noniid_client_details.csv",
)
...
class AdaptiveQuantizedFedAvg(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        for _, fit_res in results:
            ...
            network_score = float(metrics.get("network_score", 0.0))
            training_score = float(metrics.get("training_score", 0.0))
            final_score = float(metrics.get("final_score", 0.0))
            loss_change = float(metrics.get("loss_change", 0.0))

            client_upload_comm = int(metrics.get("upload_comm_bytes", 0))
            client_download_comm = int(metrics.get("download_comm_bytes", 0))
            client_total_comm = int(metrics.get("total_comm_bytes", 0))

            communication_reduction = float(
                metrics.get("communication_reduction", 0.0)
            )

            estimated_upload_time = float(
                metrics.get("estimated_upload_time", 0.0)
            )

            estimated_total_time = float(
                metrics.get("estimated_total_time", 0.0)
            )
            ...
            weighted_training_score += training_score * num_examples
            weighted_final_score += final_score * num_examples
        ...
        avg_training_score = weighted_training_score / total_examples
        avg_final_score = weighted_final_score / total_examples
        ...
        print()
        print("=" * 70)
        print(f"[Adaptive Non-IID] Round {server_round} Fit")
    ...
    def aggregate_evaluate(
        ...
        upload_comm_bytes = fit_metrics.get("upload_comm_bytes", 0)
        download_comm_bytes = fit_metrics.get("download_comm_bytes", 0)
        total_comm_bytes = fit_metrics.get("total_comm_bytes", 0)
        ...
        with open(
            ...
            writer.writerow(
                [
                    server_round,
                    "adaptive_noniid",
                    fit_metrics.get("num_clients", len(results)),
                    fit_metrics.get("total_examples", total_examples),
                    accuracy,
                    test_loss,
                    fit_metrics.get("avg_train_loss", 0.0),
                    fit_metrics.get("avg_train_time", 0.0),
                    fit_metrics.get("avg_bit_width", 0.0),
                    fit_metrics.get("avg_network_score", 0.0),
                    fit_metrics.get("avg_training_score", 0.0),
                    fit_metrics.get("avg_final_score", 0.0),
                    fit_metrics.get("avg_loss_change", 0.0),
                    fit_metrics.get("avg_bandwidth_mbps", 0.0),
                    fit_metrics.get("avg_latency_ms", 0.0),
                    upload_comm_bytes,
                    download_comm_bytes,
                    total_comm_bytes,
                    fit_metrics.get("avg_reduction", 0.0),
                    fit_metrics.get("avg_estimated_upload_time", 0.0),
                    fit_metrics.get("avg_estimated_total_time", 0.0),
        ...
        print()
        print("=" * 70)
        print(f"[Adaptive Non-IID] Round {server_round} Evaluation")
...
print()
print("=" * 70)
print("Adaptive Quantization Non-IID Server Started")
print("Dataset: MNIST")
print("Data split: label-based Non-IID")
